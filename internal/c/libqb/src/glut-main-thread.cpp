...
static void initialize_glut() {
    if (!GLUTEmu_WindowCreate(window_title ? reinterpret_cast<const char *>(window_title) : "Untitled", 640, 400,
                              GLUTEmu_WindowFlag(GLUTEmu_WindowFlag::OpenGL | GLUTEmu_WindowFlag::ScaleToMonitor | GLUTEmu_WindowFlag::AllowDND |
                                                 (screen_hide ? GLUTEmu_WindowFlag::Hide : GLUTEmu_WindowFlag::None)))) {
    ...
    if (GLEW_OK != glewError) {
        gui_alert(reinterpret_cast<const char *>(glewGetErrorString(glewError)));
        exit(EXIT_FAILURE);
    }
